...
void loop() {
  ...
  if (measuredWater > 0) {  // Check of er een nieuwe meting heeft plaatsgevonden;

    if (wifiStatus == 0) {  // Hier wordt eindelijk van wifiStatus gebruik gemaakt om een extra WiFi.status() functie te vermeiden.
      wifiConnect();
    }
...
void wifiShutdown() {  // Checkt of de tijd voorbij is en WiFi aan staat.
